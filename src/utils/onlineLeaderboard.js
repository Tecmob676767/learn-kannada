...
export const syncUserToCloud = async (userData) => {
  ...
  try {
    ...
    const userRecord = {
      ...
      googleId: userData.googleId || currentGlobal[cleanCode]?.googleId || null,
      email: userData.email || currentGlobal[cleanCode]?.email || null,
      ...
      settings: userData.settings || currentGlobal[cleanCode]?.settings || { theme: 'standard' },
      lastActive: Date.now(),
